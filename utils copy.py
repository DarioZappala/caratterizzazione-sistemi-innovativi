...
def filterYearPast(df, date_column, year_s, delta=5):
    # Convert the date column to datetime if not already
    if not pd.api.types.is_datetime64_any_dtype(df[date_column]):
        df[date_column] = pd.to_datetime(df[date_column])
    ...
    # Create the start and end date ranges
    year = pd.Timestamp(year=year_s, month=12, day=31)
    past = pd.Timestamp(year=year_s-delta, month=12, day=31)

    # Filter the DataFrame based on the date range
    filtered_df = df[(df[date_column] >= past) & (df[date_column] <= year)]
